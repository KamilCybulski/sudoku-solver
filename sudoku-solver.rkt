...
;; Board -> Board | false
;; produce a solution for a given board; produce false if the board is unsolvable
...
;; Board -> Boolean
;; ASSUME: Board is always valid
;; Produces true if the board is solved (i.e. all fields have values)
...
;; Board -> (listof Board)
;; Given a Board, produce a list of valid next boards
;; produce == find first empty field and fill it with Natural[1, 9], keeps only valid boards
(check-expect (next-boards BD5) (list
                                 (list 5 2 B B B 4 B 7 B
                                       B 1 B B 5 B 6 B B
                                       B B 4 9 B B B B B
                                       B 9 B B B 7 5 B B
                                       1 8 B 2 B B B B B 
...
(check-expect (next-boards BD4) (list
...
(define (next-boards bd)
  (keep-valid (make-boards bd (find-blank bd))))


;; Board -> Position
;; Produce the position of the first blank square on the board
;; ASSUME: There is always a blank square on the board
(check-expect (find-blank BD1) 0)
(check-expect (find-blank BD2) 9)
(check-expect (find-blank BD5) 1)
...
;; Produce a list of 9 new boards with values [1-9] filled at a given position
(check-expect (make-boards BD1 0) (list
                                   (fill-square BD1 0 1)
                                   (fill-square BD1 0 2)
                                   (fill-square BD1 0 3)
                                   (fill-square BD1 0 4)
...
(check-expect (make-boards BD4 3) (list
                                   (fill-square BD4 3 1)
                                   (fill-square BD4 3 2)
                                   (fill-square BD4 3 3)
                                   (fill-square BD4 3 4)
...
(define (make-boards bd p)
  (local [(define (maker bd p n)
            (cond [(> n 9) empty]
                  [else
                   (cons (fill-square bd p n) (maker bd p (+ n 1)))]))]
...
;; boards after one step (for testing purposes)
...
;; Produce a list of only valid boards
(check-expect (keep-valid LOB1) LOB1)
(check-expect (keep-valid LOB4) (list
                                 (fill-square BD4 3 6)
                                 (fill-square BD4 3 8)))
...
(check-expect (valid? (fill-square BD7 8 1)) false)
(check-expect (valid? (fill-square BD5 1 5)) false)

(define (valid? bd)
  (local [(define (get-units units)
            (map get-unit units))

          (define (get-unit unit)
            (map transform unit))

          (define (transform e)
                (read-square bd e))

          (define (check-units lou)
            (andmap check-unit lou))

          (define (check-unit lon)
            (local [(define (unique? e)
                      (cond [(false? e) true]
                            [else (not (member? e (remove e lon)))]))]
              
              (andmap unique? lon)))
          ]

    (check-units (get-units UNITS))))
